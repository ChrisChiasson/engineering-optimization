...
TableForm[tableau[4,4,a]={{2,1,-1,0,2},{2,4,0,-1,4},{2,4,0,0,F}}]
...
TableForm[tableau[4,4,e]=Delete[Map[Drop[#,{5,6}]&,tableau[4,4,d],1],-1]]


(*notice that the problem is already optimized, 
  so this next call changes nothing*)
TableForm[tableau[4,4,f]=
	LinearMinimizeTableau[tableau[4,4,e],{{1,1},{2,2}}]
	]
      ...
      ReleaseHold@
        Hold[ContourPlot][eqns[4,4,1][X@1,X@2][[2]],Sequence@@rangeSpec[4,4],
			ColorFunction->(Hue[.7,1-#,1]&),
				version6[PlotPoints->30,
					RegionFunction->(regionFunction[4,4][#1,#2]&),Contours->
						Sort@{0.56,2.85,5.15,7.44,-1.73,-4.02,-6.31,9.73,12.02,
							14.31}]];
    gr[4,4,2]=
		...
		With[{solVector={X@1,X@2}/.sol[4,4][[2]]},
			Graphics[
				{Thickness[0.01],Dashing[{.05,.025}],Red,
					...
					Text[
						DisplayCell[
							NumberForm[eqns[4,4,1][Sequence@@#][[2]],2],
							Background->White],
...
TableForm[tableau[4,5,a]=
	{{2,1,-1,0,2},{2,1,0,-1,-1},{2,4,0,0,F}}
	]

exportTableau["P 4-5 Initial Tableau",table[4,5,a],Automatic,None];


TableForm[tableau[4,5,b]=
	{{2,1,-1,0,1,2},{-2,-4,0,1,0,1},{2,4,0,0,0,F},{0,0,0,0,1,w}}
	]

exportTableau["P 4-5 Augmented Tableau",table[4,5,b],Automatic,None];


TableForm[tableau[4,5,c]=
...
TableForm[tableau[4,5,d]=
	LinearMinimizeTableau[tableau[4,5,c],{{1,5},{2,4}}]
	]
...
TableForm[tableau[4,5,e]=
	Delete[Map[Delete[#,5]&,tableau[4,5,d],1],-1]
	]


(*notice that the problem is already optimized, so this next call does \
nothing*)
TableForm[tableau[4,5,f]=
	LinearMinimizeTableau[tableau[4,5,e],{{1,1},{2,4}}]
	]
    ...
    gr[4,5,1]=
      ReleaseHold@
        Hold[ContourPlot][eqns[4,5,1][X@1,X@2][[2]],Sequence@@rangeSpec[4,5],
          ColorFunction->(Hue[.7,1-#,1]&),
          	version6[PlotPoints->30,
					RegionFunction->(regionFunction[4,5][#1,#2]&),Contours->
